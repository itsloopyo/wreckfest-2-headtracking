...
void PositionLimitTests() {
    ...
    // The typo worth catching is a misplaced decimal point, not a mistyped
    // 10000: 4.0 for 0.40 puts the render eye four metres outside the car, and
    // a metres-wide bound would pass it through untouched and unlogged.
    Check(SanitizePositionLimit(4.0f, 0.40f) == 0.5f, "a misplaced decimal point clamps");
    Check(SanitizePositionLimit(3.0f, 0.30f) == 0.5f, "and so does the whole-number form");
...
int main() {
    std::printf("Wreckfest 2 head tracking - config boundary tests\n");
    std::printf("======================================================\n");
    SmoothingTests();
    PositionLimitTests();
    VirtualKeyTests();
